...
const AdminDashboardPage: React.FC = () => {
    ...
    useEffect(() => {
        if (isAdmin && user) { // Ensure user is available for token
            const fetchStats = async () => {
                ...
                try {
                    ...
                    const filesResponse = await fetch('/api/admin/stats/files', { headers });
                    if (!filesResponse.ok) {
                    ...
                    const firestoreResponse = await fetch('/api/admin/stats/firestore', { headers });
                    if (!firestoreResponse.ok) {
                        const errorData = await firestoreResponse.json();
                        throw new Error(errorData.error || 'Failed to fetch Firestore stats');
                    }
                    const firestoreData = await firestoreResponse.json();

                    const usersResponse = await fetch('/api/admin/stats/users', { headers });
                    if (!usersResponse.ok) {
            ...
            {statsLoading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <div className="bg-white shadow-lg rounded-lg p-6">
                        <h2 className="text-xl font-semibold mb-2">Total Files in Bucket</h2>
                        <p className="text-4xl font-bold">{stats.totalFiles ?? 'N/A'}</p>
                    </div>
                    <div className="bg-white shadow-lg rounded-lg p-6">
                        <h2 className="text-xl font-semibold mb-2">Total Data in Firestore (File Metadatas)</h2>
                        <p className="text-4xl font-bold">{stats.totalFirestoreData ?? 'N/A'}</p>
                    </div>
                    <div className="bg-white shadow-lg rounded-lg p-6">
