...
export default function AdminFeedback() {
    ...
    const [filter, setFilter] = useState<'all' | 'new' | 'in-progress' | 'resolved' | 'closed'>('all');
    ...
    const updateFeedbackStatus = async (id: string, newStatus: FeedbackItem['status']) => {
        try {
                      ...
                      ${item.status === 'new' ? 'bg-yellow-100 text-yellow-800' :
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                        <div className="flex space-x-2">
                                            <select
                                                value={item.status}
                                                onChange={(e) => updateFeedbackStatus(item.id, e.target.value as FeedbackItem['status'])}
