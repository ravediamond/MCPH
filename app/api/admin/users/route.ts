...
export async function GET(request: Request) {
    try {
        ...
        const usersWithCratesPromise = firestore.collection("crates")
            .select("userId")
            .get()
            .then(snapshot => {
                ...
                snapshot.docs.forEach(doc => {
                    ...
                    if (userId) {
                        cratesCountByUser.set(userId, (cratesCountByUser.get(userId) || 0) + 1);
                    }
                });
        ...
        const mcpCallsCountByUserPromise = firestore.collection("mcpCalls")
            .select("userId")
            .get()
            .then(snapshot => {
                ...
                snapshot.docs.forEach(doc => {
                    const userId = doc.data().userId;
                    if (userId) {
                        mcpCallsCountByUser.set(userId, (mcpCallsCountByUser.get(userId) || 0) + 1);
                    }
                });
        ...
        const storageUsedByUserPromise = firestore.collection("crates")
            .get()
            .then(snapshot => {
        ...
        const userActivityPromise = firestore.collection("userActivity")
            .get()
            .then(snapshot => {
                ...
                snapshot.docs.forEach(doc => {
                    ...
                    if (data.lastActive && data.userId) {
                        const timestamp = data.lastActive.toDate ? data.lastActive.toDate() : new Date(data.lastActive);
        ...
        const formattedUsers = firebaseUsers.map(firebaseUser => {
            ...
            return {
                ...
                lastActive: lastActiveByUser.get(firebaseUser.uid) || firebaseUser.metadata.lastSignInTime
            };
        });
