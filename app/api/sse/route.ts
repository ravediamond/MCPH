  ...
  try {
      ...
      if (usage.count > 1000) {
        send({
          ...
          error: {
            code: 42901,
            message:
              "Monthly tool call quota exceeded (1000/month per account).",
    ...
    switch (rpc.method) {
          ...
          result: {
            tools: [
              ...
              },
              {
                ...
                description:
                  "Upload a new artifact. For binary files, returns a presigned upload URL. For text, uploads directly.",
                inputSchema: {
                  type: "object",
                  properties: {
                    ...
                    data: {
                      type: "string",
                      description:
                        "Base64-encoded file content (for text files)",
                    },
              ...
              },
              {
                ...
                description:
                  "Make an artifact shareable (public link) and optionally set/remove a password. Returns the shareable link and sharing status.",
                inputSchema: {
                  type: "object",
                  properties: {
                    id: { type: "string" },
      ...
      case "tools/call":
        if (!(await enforceRateLimit())) break;
        console.log(
          `[SSE] tools/call: ${rpc.params?.name}`,
        ...
        if (rpc.params?.name === "artifacts/list") {
          ...
          if (!parsed.success) {
            console.warn(
              "[SSE] Invalid params for artifacts/list",
              parsed.error.issues,
            );
            send({
              jsonrpc: "2.0",
          ...
          try {
            ...
            console.log(
              `[SSE] artifacts/list returned ${artifacts.length} artifacts`,
            );
            send({
              jsonrpc: "2.0",
              id: rpc.id,
        ...
        if (rpc.params?.name === "artifacts/get") {
          ...
          if (!parsed.success) {
            console.warn(
              "[SSE] Invalid params for artifacts/get",
              parsed.error.issues,
            );
            send({
              jsonrpc: "2.0",
        ...
        if (rpc.params?.name === "artifacts/get_metadata") {
          ...
          if (!parsed.success) {
            console.warn(
              "[SSE] Invalid params for artifacts/get_metadata",
              parsed.error.issues,
            );
            send({
              jsonrpc: "2.0",
          ...
          try {
            ...
            if (!meta) {
              ...
              console.log(
                `[SSE] artifacts/get_metadata returned artifact for id: ${parsed.data.id}`,
              );
              send({
                jsonrpc: "2.0",
                id: rpc.id,
            ...
            send({
              ...
              error: {
                code: -32000,
                message: "Failed to get artifact metadata",
                data: String(err),
              },
            });
          }
        ...
        if (rpc.params?.name === "artifacts/search") {
          ...
          try {
            ...
            const vectorArtifacts = vectorSnapshot.docs.map((doc) => ({
              id: doc.id,
              ...doc.data(),
            }));
            ...
            const classicalSnapshot = await filesRef
              ...
              .limit(topK)
              .get();
            const classicalArtifacts = classicalSnapshot.docs.map((doc) => ({
              id: doc.id,
              ...doc.data(),
            }));
        ...
        if (rpc.params?.name === "artifacts/upload") {
          ...
          if (!parsed.success) {
            console.warn(
              "[SSE] Invalid params for artifacts/upload",
              parsed.error.issues,
            );
            send({
              jsonrpc: "2.0",
            ...
            if (
              ...
              send({
                ...
                result: {
                  ...
                  gcsPath,
                  message:
                    "Upload your file using this URL with a PUT request.",
                },
              });
            } else {
              ...
              if (!data) {
                send({
                  ...
                  error: {
                    code: -32602,
                    message: "Missing data for text upload",
                  },
                });
                break;
              }
