...
export async function GET(
    ...
    try {
        ...
        if (error.message && error.message.toLowerCase().includes("not found")) {
            return NextResponse.json({ error: "Crate not found or has expired" }, { status: 404 });
        }
        // Differentiate other errors if possible, e.g., storage service unavailable
        if (error.message && error.message.toLowerCase().includes("failed to get crate content")) {
            return NextResponse.json({ error: "Failed to retrieve crate content from storage" }, { status: 503 });
        }

        return NextResponse.json(
            { error: "Failed to download crate", details: error.message || "Unknown error" },
            { status: 500 }
        );
    }
}
