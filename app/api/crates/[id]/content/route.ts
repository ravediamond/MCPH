...
export async function GET(
    ...
    try {
        ...
        console.log(`[Content Route] Found crate: ${crate.title}, Owner: ${crate.ownerId}, Public: ${crate.shared.public}`);
        ...
        if (now > expirationDate) {
            console.log(`[Content Route] Crate expired. Created: ${crate.createdAt}, TTL: ${crate.ttlDays} days`);
        ...
        console.log(`[Content Route] Session cookie present: ${Boolean(sessionCookie)}`);
        ...
        if (authHeader && authHeader.startsWith("Bearer ")) {
            ...
            try {
                ...
                console.log(`[Content Route] Token auth successful. User ID: ${userId}`);
            } catch (error) {
                console.warn(`[Content Route] Invalid authentication token:`, error);
            }
        } else {
            console.log(`[Content Route] No valid Bearer token found. Checking session auth.`);
            ...
            if (sessionCookie && sessionCookie.value) {
                try {
                    ...
                    isAuthenticated = true;
                    console.log(`[Content Route] Session auth successful. User ID: ${userId}`);
                } catch (error) {
        ...
        console.log(`[Content Route] Access check - isOwner: ${isOwner}, isPublic: ${isPublic}, isSharedWithUser: ${isSharedWithUser}`);
        console.log(`[Content Route] Current user ID: ${userId}, Crate owner ID: ${crate.ownerId}`);
        ...
        if (id === "031a5b31-4bcb-47de-bde9-c39a1cbb2297") {
            ...
            console.log(`[Content Route] Content retrieved successfully, size: ${buffer.length} bytes`);
        ...
        if (isPublic && crate.shared.passwordProtected && !isOwner) {
            console.log(`[Content Route] Crate is password protected and user is not owner`);
        ...
        if (!isOwner && !isPublic && !isSharedWithUser) {
            console.log(`[Content Route] Permission denied. Not owner, not public, not shared with user.`);
            return NextResponse.json(
