...
import { FaUpload, FaFile, FaSpinner, FaCheckCircle, FaCopy, FaExternalLinkAlt, FaArrowRight } from "react-icons/fa";
...
export default function HeroUpload() {
  ...
  const handlePaste = (e: React.ClipboardEvent) => {
    ...
    if (text.trim()) {
      ...
      const textFile = new File([text], "pasted-text.md", { type: "text/markdown" });
      setFile(textFile);
            ...
            {isUploading ? (
              <div className="space-y-6">
                ...
                <div className="w-full max-w-xs mx-auto">
                  ...
                  <p className="text-gray-700 mt-3 font-medium">Uploading: {uploadProgress}%</p>
                  {file && <p className="text-sm text-gray-600 mt-1 truncate max-w-full">{file.name}</p>}
                </div>
              </div>
            ) : (
              <div className="space-y-4 md:space-y-5">
                ...
                {isDragActive ? (
                  <p className="text-orange-500 text-lg md:text-xl font-medium">Drop your file here...</p>
                ) : (
                  <>
                    <div>
            ...
            <div className="relative">
              ...
              <div className="absolute right-1.5 top-1.5 flex">
                <button
                  ...
                  className={`${
                    urlCopied ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-700 hover:bg-gray-200"
                  } p-1.5 md:p-2 rounded-md mr-1 md:mr-1.5 transition-colors duration-200 text-sm`}
                  aria-label="Copy link to clipboard"
                >
                  {urlCopied ? <span className="text-xs md:text-sm">Copied!</span> : <FaCopy />}
                </button>
                <a
                  href={uploadedUrl}
                  target="_blank"
            ...
            <div className="text-center mt-3 md:mt-4 p-2 md:p-3 bg-blue-50 rounded-lg">
              {!user ? (
                <p className="text-xs md:text-sm text-blue-700">
                  This link will expire in 30 days. <a href="/login" className="font-medium underline">Create an account</a> to manage your uploads.
                </p>
              ) : (
                <div className="space-y-2">
                  <p className="text-xs md:text-sm text-blue-700">
                    Need more options? Use the advanced uploader for metadata, tags, and batch uploads.
                  </p>
                  <Link 
                    href="/upload" 
