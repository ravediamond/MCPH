...
const serviceAccountPath = process.env.FIREBASE_ADMIN_SDK_SERVICE_ACCOUNT_PATH || './service-account-credentials.json';
    ...
    try {
        ...
        if (process.env.VERCEL && process.env.FIREBASE_ADMIN_SDK_PRIVATE_KEY) {
            ...
            if (!fs.existsSync(resolvedServiceAccountPath)) {
                ...
                console.error('[FirebaseAdmin] Please ensure FIREBASE_ADMIN_SDK_SERVICE_ACCOUNT_PATH is set correctly or the file exists at the default location.');
        ...
        initialized = true;
        console.log('[FirebaseAdmin] Initialization successful. Flag "initialized" set to true.');
    } catch (error: any) {
